...
class Home extends CI_Controller {
	...
	public function index(){
        ...
        $data['seo'][0] = array(
    		'meta_title'=>'Advanced Fatty Liver & NASH Treatment | ReMASH India',
    		'meta_description'=> 'Explore ReMASH for cutting-edge solutions to fatty liver, NASH, and cirrhosis. Discover advanced liver care medications like Rezdiffra and Resmetirom in India.',
    		'meta_keywords'=>'fatty liver medication,best medicine for liver,fatty liver treatment,NASH treatment,resmetirom,rezdiffra tablet,liver fibrosis treatment,medicine for liver cirrhosis,fatty liver medicine in India,advanced liver care,non-alcoholic fatty liver disease treatment,hepatic fibrosis solution',
		);
...
//          $data['seo'][0] = array(
//     		'meta_title'=>'ReMASH',
//     		'meta_description'=> 'ReMASH',
//     		'meta_keywords'=>'ReMASH',
// 		);
...
// 		);
	...
	public function prescribing_information(){
		$data = array();
        $data['active_item'] = 'prescribing-information';
// 	    $data['seo'][0] = array(
//     		'meta_title'=>'ReMASH',
//     		'meta_description'=> 'ReMASH',
//     		'meta_keywords'=>'ReMASH',
// 		);
		//echo "<pre>"; print_r($data['seo']); exit;
		$this->load->view('prescribing_information', $data);
	}
	public function awareness_hub(){
		$data = array();
        $data['active_item'] = 'awareness_hub';
// 	    $data['seo'][0] = array(
//     		'meta_title'=>'',
//     		'meta_description'=> '',
//     		'meta_keywords'=>'',
// 		);
		//echo "<pre>"; print_r($data['seo']); exit;
		$this->load->view('awareness_hub', $data);
	}
	...
	public function leadformsubmission(){
        ...
        if($status['success']){
            ...
            $this->form_validation->set_rules('enquiry_subject','Please Enter your subject','trim|required|min_length[3]|max_length[600]');
            ...
            if($this->form_validation->run()==False){
                ...
                $contact_number = $this->input->post('enquiry_contact');
                $full_contact = $country_code . ' ' . $contact_number;
                $add_enquiry = array('enquiry_name' => $this->input->post('enquiry_name'),
                              'enquiry_mail' => $this->input->post('enquiry_mail'),
                              'enquiry_contact' => $full_contact,
                ...
                $insert_enquiry = $this->Home_model->insertData('db', 'remash_enquiry_from', $add_enquiry);
                // echo $insert_enquiry; exit;
                if($insert_enquiry > 0){
                    $to_name = $this->input->post('enquiry_name');
                    $to = $this->input->post('enquiry_mail');
                    $subject = "ReMASH Enquiry";
                    ...
                    <body>
                        ...
                        <p>Hello <b>".$this->input->post('enquiry_name')."</b>,<br><br>
                            ...
                            <tr><td>Phone:</td><td>" .$full_contact. "</td></tr>
                            <tr><td>Subject:</td><td>" .$this->input->post('enquiry_subject'). "</td></tr>
    ...
    public function prescribing_information_form_submission() {
        $lead_name    = $this->input->post('lead_name');
        $lead_contact = $this->input->post('lead_contact');
        $country_code = $this->input->post('country_code');
        $lead_mail    = $this->input->post('lead_mail');
        $lead_country = $this->input->post('lead_country');
    
        $full_contact = $country_code . ' ' . $lead_contact;
    
        $data = array(
            'lead_name'    => $lead_name,
            'lead_contact' => $full_contact,
            'lead_mail'    => $lead_mail,
            'lead_country' => $lead_country,
            'created_at'   => date('Y-m-d H:i:s')
        );
    
        $this->db->insert('prescribing_info_leads', $data);
    
        // Prepare response
        $response = array(
            'pdf_url' => base_url('uploads/prescribing_information/prescribing_information.pdf'),
            'redirect_url' => base_url("success/prescribing-information/" . date('Y-m-d') . "-" . uniqid())
        );
    ...
    public function success(){  
        ...
        elseif($this->uri->segment(2) == "prescribing-information"){
            // $data['msg1'] = "Thanks for downloading the ReMASH Prescribing Information leaflet.";
            $data['msg2'] = "Thank you for accessing the ReMASH Prescribing Information leaflet.";
        }
